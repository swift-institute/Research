...
2. The workspace's actual on-disk state — `~/.claude/settings.json`,
   ...
   it.
3. CLAUDE.md skill-routing tables (`[local-workspace]/CLAUDE.md`)
...
[Verified: 2026-05-10 via `cat ~/.claude/settings.json`]

**User-level local additions** (`~/.claude/settings.local.json`):
...
**Project-level local** (`[local-workspace]/.claude/settings.local.json`,
...
`[local-workspace]/.claude/skills/`, regenerated by
...
| **Total symlinked** | **54** | `ls [local-workspace]/.claude/skills/ \| wc -l` |

[Verified: 2026-05-10 via `ls [local-workspace]/swift-institute/Skills/
\| wc -l` (47 entries minus LICENSE.md and README.md = 45) and `ls
[local-workspace]/.claude/skills/ \| wc -l` (54)]

**Memory layer**:
- Auto-memory directory:
  `~/.claude/projects/-Users-coen-Developer/memory/` — 188 files
...
- Project-level CLAUDE.md (workspace-wide instructions):
  `[local-workspace]/CLAUDE.md` (~330 lines) — skill routing tables,
  collaboration protocol, layer architecture, package locations, gotchas.
- User-level CLAUDE.md: `~/CLAUDE.md` — short utility file
...
- `swift-institute/.github/.github/workflows/`: 27 reusable workflows
  ...
  [local-workspace]/swift-institute/.github/.github/workflows/`].
...
- Per-package `ci.yml` files in swift-primitives subrepos (372 files
  ...
  `[local-workspace]/swift-primitives/`): each is a thin wrapper
  delegating to the centralized reusables [Verified: 2026-05-10 via `find
  [local-workspace]/swift-primitives -path "*/.github/workflows/ci.yml"
...
- No user-level `agents/` directory (no project-defined sub-agents on top
  ...
  `statusline-setup`) [Verified: 2026-05-10 via `ls ~/.claude/`
...
- No user-level or project-level `hooks/` directory [Verified: 2026-05-10
  via `find ~/.claude/hooks [local-workspace]/.claude/hooks
...
| Aspect | Workspace state |
|--------|-----------------|
| What ships | Native Read / Write / Edit / Grep / Glob; durable state via the user's normal git workflow across ~30+ sibling repos under `[local-workspace]/`; per-package `Research/`, `Audits/`, `Experiments/` directories codified by [RES-002], [RES-002a], [META-*]. |
...
3. **Memory — Discipline (§3.7).** MEMORY.md exceeds the 24.4KB cap;
   ...
   `[local-workspace]/CLAUDE.md`) is being violated by the index's
...
3. **Hooks-layer absence — bet or gap? (Component 10 / §3.10).** The
   ...
   workspace's *Collaboration Protocol* (`[local-workspace]/CLAUDE.md`)
...
[Verified: 2026-05-10 via `[local-workspace]/CLAUDE.md`] read as
...
`[local-workspace]/CLAUDE.md`. Tier 2 or Tier 3 candidate for
...
- `~/.claude/settings.local.json` — user-level local additions
- `[local-workspace]/.claude/settings.local.json` — project-level local
- `[local-workspace]/CLAUDE.md` — workspace instructions, skill routing tables
- `~/CLAUDE.md` — user-level utility CLAUDE.md
- `~/.claude/cclsp.json` — cclsp MCP server configuration
- `~/.claude/projects/-Users-coen-Developer/memory/MEMORY.md` — auto-memory index
- `~/.claude/projects/-Users-coen-Developer/memory/*.md` — 188 topic files
- `[local-workspace]/swift-institute/Skills/{handoff,supervise,reflect-session,reflections-processing,skill-lifecycle,swift-institute-core,code-surface,swift-institute,corpus-meta-analysis,ci-cd-workflows,git-operations}/SKILL.md`
- `[local-workspace]/swift-institute/.github/.github/workflows/` — 27 reusable workflows
- `[local-workspace]/swift-primitives/.github/.github/workflows/swift-ci.yml` — layer-tier reusable
- `[local-workspace]/swift-institute/Scripts/` — 28 workspace tooling scripts
- `[local-workspace]/swift-institute/Research/agent-harness-engineering-state-of-the-art.md` (the survey)
- `[local-workspace]/swift-institute/Research/agent-handoff-patterns.md`
- `[local-workspace]/swift-institute/Research/agent-supervision-patterns.md`
- `[local-workspace]/swift-institute/Research/ai-context-reduction-via-type-system-tooling.md`
- `[local-workspace]/swift-institute/Research/claude-code-swift-rewrite-feasibility.md` (DEFERRED)
