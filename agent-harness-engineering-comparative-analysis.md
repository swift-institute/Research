...
2. The workspace's actual on-disk state — `~/.claude/settings.json`,
   ...
   it.
3. CLAUDE.md skill-routing tables (`~/Developer/CLAUDE.md`)
...
**Project-level local** (`~/Developer/.claude/settings.local.json`,
...
`~/Developer/.claude/skills/`, regenerated by
...
| **Total symlinked** | **54** | `ls ~/Developer/.claude/skills/ \| wc -l` |

[Verified: 2026-05-10 via `ls ~/Developer/swift-institute/Skills/
\| wc -l` (47 entries minus LICENSE.md and README.md = 45) and `ls
~/Developer/.claude/skills/ \| wc -l` (54)]
...
- Project-level CLAUDE.md (workspace-wide instructions):
  `~/Developer/CLAUDE.md` (~330 lines) — skill routing tables,
...
- `swift-institute/.github/.github/workflows/`: 27 reusable workflows
  ...
  ~/Developer/swift-institute/.github/.github/workflows/`].
...
- Per-package `ci.yml` files in swift-primitives subrepos (372 files
  ...
  `~/Developer/swift-primitives/`): each is a thin wrapper
  delegating to the centralized reusables [Verified: 2026-05-10 via `find
  ~/Developer/swift-primitives -path "*/.github/workflows/ci.yml"
...
- No user-level or project-level `hooks/` directory [Verified: 2026-05-10
  via `find ~/.claude/hooks ~/Developer/.claude/hooks
...
| Aspect | Workspace state |
|--------|-----------------|
| What ships | Native Read / Write / Edit / Grep / Glob; durable state via the user's normal git workflow across ~30+ sibling repos under `~/Developer/`; per-package `Research/`, `Audits/`, `Experiments/` directories codified by [RES-002], [RES-002a], [META-*]. |
...
3. **Memory — Discipline (§3.7).** MEMORY.md exceeds the 24.4KB cap;
   ...
   `~/Developer/CLAUDE.md`) is being violated by the index's
...
3. **Hooks-layer absence — bet or gap? (Component 10 / §3.10).** The
   ...
   workspace's *Collaboration Protocol* (`~/Developer/CLAUDE.md`)
...
[Verified: 2026-05-10 via `~/Developer/CLAUDE.md`] read as
...
`~/Developer/CLAUDE.md`. Tier 2 or Tier 3 candidate for
...
- `~/Developer/.claude/settings.local.json` — project-level local
- `~/Developer/CLAUDE.md` — workspace instructions, skill routing tables
...
- `~/Developer/swift-institute/Research/ai-context-reduction-via-type-system-tooling.md`
- `~/Developer/swift-institute/Research/claude-code-swift-rewrite-feasibility.md` (DEFERRED)
