...
**Key finding**: CCLSP's workspace-level features (`find_workspace_symbols`) require SourceKit-LSP to have a Package.swift at or above the working directory. The CWD `~/Developer` has no Package.swift, so workspace queries return empty. File-targeted queries (`find_definition`) work because they trigger single-file indexing.
...
Add a `Package.swift` at `~/Developer` that declares all ecosystem packages as local dependencies.
...
```
~/Developer/Package.swift          → all layers (for occasional full-index)
  or per-layer:
...
- If viable, add a root-level `~/Developer/Package.swift` that unifies all layers
