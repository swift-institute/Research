...
- `markdown-rendering-organization-audit.md` — F-6 identifies AnyView as architectural
- `swift-institute/Research/rendering-view-associated-type-naming.md` — Body rename enabling SwiftUI previews
...
### Approach C: Markdown as Leaf View with Body == Never
...
- **C** (Leaf View): `Markdown` becomes a leaf view (`Body = Never`) with a custom `_render<C>`.
...
1. `Markdown.swift` — change from `body`-based rendering to `Body = Never` + `_render<C>`
...
- `markdown-rendering-organization-audit.md` — organizational findings F-1 through F-10
- `swift-institute/Research/rendering-view-associated-type-naming.md` — Body rename (prerequisite)
