...
**File**: `~/Developer/swift-primitives/swift-path-primitives/Sources/Path Primitives/Path.String.swift`
...
After fixing:
```bash
cd ~/Developer/swift-primitives
swift build -c release 2>&1 | grep -E "(error|Build complete)"
```

Then verify the nested test package can build in release:
```bash
cd ~/Developer/swift-institute/Experiments/rendering-context-protocol-vs-witness/Tests
rm -rf .build
...
**swift-testing location**: `~/Developer/swift-foundations/swift-testing/`

Search for:
```bash
grep -rn "PERFORMANCE_DIAGNOSTIC" \
  ~/Developer/swift-foundations/swift-testing/ \
  --include="*.swift"
```
...
| File | Contains |
|------|----------|
| `~/Developer/swift-primitives/swift-path-primitives/Sources/Path Primitives/Path.String.swift` | Crashing `defer { for buffer in unsafe ... }` pattern (lines 562–579) |

### swift-testing

| File | Contains |
|------|----------|
| `~/Developer/swift-foundations/swift-testing/` | Root of swift-testing package |
...
| File | Contains |
|------|----------|
| `~/Developer/swift-institute/Experiments/rendering-context-protocol-vs-witness/Tests/Performance Tests/RenderingDispatchPerformanceTests.swift` | Test file that demonstrates all three swift-testing issues |
| `~/Developer/swift-institute/Experiments/rendering-context-protocol-vs-witness/Sources/Benchmark/main.swift` | Ad-hoc benchmark that the swift-testing fixes should make unnecessary |
...
| Package | Path |
|---------|------|
| swift-primitives (monorepo) | `~/Developer/swift-primitives/` |
| swift-path-primitives | `~/Developer/swift-primitives/swift-path-primitives/` |
| swift-testing | `~/Developer/swift-foundations/swift-testing/` |
| swift-institute | `~/Developer/swift-institute/` |
| Experiment (reference) | `~/Developer/swift-institute/Experiments/rendering-context-protocol-vs-witness/` |
