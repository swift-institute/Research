...
| Whether the integration-overstatement factor (~10× in the swift-json case) generalises across consumer shapes or is workload-specific | **premise** (would change §5's recommendation strength if disproven) | Per [RES-027], premise items normally require an extant or immediately-created experiment. **Extant experiment**: the `parse-performance-bench` harness already captures the overstatement at one consumer shape. Generalisation across consumer shapes is an open question; the integration-probe requirement in §5 is the durable response that does NOT depend on knowing the factor in advance. The rule fires whether the factor is 10× or 2×. |
