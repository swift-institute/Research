...
> **SUPERSEDED (2026-05-31, per principal).** Retained in place per [META-005] (no `_archived/`; hidden from the active view by the `_index.json` `status: SUPERSEDED` filter) as an audit trail — **do not act on the Option-C "remove the types" recommendation below.** Verified current source contradicts its central premise: `Storage.Pool` composes `Memory.Pool` and `Storage.Arena` composes `Memory.Arena`, both with real cross-package consumers (Tree.\*, the machines, the timer wheel). Disposition is governed by [RES-020a] (total-taxonomy: L1 types are justified by lattice position, not consumer count). See `memory-buffer-allocator-institute-vs-apple-comparative.md` (Appendix B).
>
