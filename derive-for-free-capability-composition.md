...
changelog:
  - "1.5.0 (2026-06-02): GAP-M LANDED — Buffer `isEmpty` element-domain dedup (GAP-E generalized to Arena/Linear/Linked/Ring; the redundant `count == .zero` overrides deleted, inheriting the gated (D) default; each base type module `@_exported`-re-exports the capability protocol so the inherited member is visible under MemberImportVisibility, propagating even to @inlinable sibling bodies). The ungated-generalization STEP-0 spike FAILED (abstract `Count` surfaces `.zero` but not `==`) and BOTH contortions — `count.underlying == .zero` (carrier-reach, rejected per [CONV-001a]) and `& Swift.Equatable` on the minimal `Count` bound (META-RULE violation) — were rejected; re-scoped to element-domain-only. Sparse fold-in DECLINED; equality lives on the element-domain gate / a refinement, never the core. Green debug+release per package (commits in §8). GAP-N spike-validated — `Buffer.Bounded.Protocol` refinement (`isFull = count == capacity` + `availableCapacity = capacity.subtract.saturating(count)`) compiles on the element-domain gate; classification narrowed it to Ring-only (Arena/Linked/Slab opt out; Linear variants don't conform `Buffer.Protocol`) → ~3 isFull copies + net-new ops, thin [MOD-RENT] rent, scope decision pending. §8 ledger updated."
  - "1.4.0 (2026-06-02): RECONCILED with the North-Star Storage.Protocol De-Pointer arc (P6/P7, merged to dev-tree — swift-storage-primitives 4aec845/677ac8f; verified: pointer(at:) gone, typed surface in, dev-tree builds green). Storage.Protocol inventory updated: REQUIRES is now {capacity, subscript{get set} via _read/_modify, initialize(at:to:), move(at:)} — pointer-free. GAP-C (Storage->span bridge) is SUPERSEDED: with pointer(at:) deleted there's no protocol-level pointer to derive a shared span from; the arc's §4 work gave Heap/Inline a per-discipline Span/MutableSpan/OutputSpan surface instead (span composes per-discipline on Memory.Contiguous.Protocol — more aligned with the model than GAP-C). §4.2/§9/§11 prose updated. (CORRECTION: an earlier draft of this entry claimed the modularization SKILL.md description 'measures 245, not blocked by sync-skills.sh' — that was itself a [RES-023] miss: I measured the content via YAML-load but did NOT run the authoritative gate. Running `sync-skills.sh --dry-run` ABORTS — `check-skill-descriptions.sh` counts the awk-extracted block via `wc -c` INCLUDING the 2-space YAML indent + newlines, so 244 content → 251 measured → over the 250 ceiling. The De-Pointer report's '251 blocker' was CORRECT; sync-skills.sh IS blocked. Lesson: run the operative tool, not a proxy. Disposition open (trim the description, or fix check-skill-descriptions.sh to count content not formatting).)"
...
| **`Storage.\`Protocol\``** (`__StorageProtocol`) | swift-storage-primitives · `Storage.Protocol.swift:20-62` | `var capacity`; `subscript(slot:) -> Element { get set }` (via `_read`/`_modify`); `initialize(at:to:)`; `move(at:)`; `Element: ~Copyable`; single-region (`Storage.Split` does NOT conform). **DE-POINTERED 2026-06-02** (North-Star P6, `4aec845`) — the legacy `pointer(at:)` requirement was deleted; the surface is now fully typed/pointer-free | **none** (no `extension __StorageProtocol`) | **none** keyed on `Storage.\`Protocol\`` — span composes **per-discipline** (Heap/Inline carry `span`/`mutableSpan`/`outputSpan`), not a protocol default. GAP-C is **SUPERSEDED**: with `pointer(at:)` gone there is no protocol-level pointer to derive a shared `span` from, and span correctly lives on `Memory.Contiguous.\`Protocol\`` per discipline | **none** |
...
1. **The cores are maximally minimal.** `Memory.Contiguous.\`Protocol\`` requires exactly `span`; `Storage`
   exactly `{capacity, subscript{get set}, initialize(at:to:), move(at:)}` (de-pointered 2026-06-02); `Buffer` exactly `count`. Each PROVIDES (D) only what its *own*
...
| **GAP-M** (Buffer `isEmpty` element-domain dedup — GAP-E generalized) — *new gap, surfaced 2026-06-02* | The redundant `count == .zero` `isEmpty` overrides deleted from the element-domain conformers (Arena ×3, Linear ×1, Linked ×3, Ring ×4) → they inherit the gated `(D)` default. Each base type module `@_exported public import`s `Buffer_Protocol_Primitives`, so the inherited member is visible to consumers under MemberImportVisibility — and it propagates even to `@inlinable` sibling bodies (Ring's Builders' `while !dynamic.isEmpty`) with no per-file import. Kept: bespoke spill-aware (`Arena.Small`), `Slab` (sparse, outside the gate), and the non-conforming Linear variants. **The sparse fold-in was DECLINED** — generalizing the default to a sparse `Count` needs `==` on the *abstract* `Count`, i.e. an equality bound on the minimal occupancy core, rejected per the META-RULE (equality may live on the element-domain gate or a refinement, never the core). The ungated `count == .zero` STEP-0 spike FAILED (abstract `Count: Carrier.\`Protocol\`<Cardinal>` surfaces `.zero` but not `==` → `Duration` mis-inference) and the `count.underlying == .zero` carrier-reach contortion was rejected per [CONV-001a]; re-scoped to element-domain-only. No `Count`-bound change; green debug+release per package. **Addendum (explicit-import sweep, 2026-06-02, principal directive "explicit > transitive"):** the `@inlinable` Builder bodies that reached the inherited `isEmpty` via the transitive `@_exported` chain were HARDENED to carry an explicit `public import Buffer_Protocol_Primitives` (public required for `@inlinable` cross-module per [MOD-027]; verified load-bearing — no unused-import warning) — so the "no per-file import" note above is superseded for those `@inlinable` sites; buffer-linear `70a056e`, buffer-ring `7f4fc67` | swift-buffer-arena-primitives `77565b6`, swift-buffer-linear-primitives `8eee6ca`/`70a056e`, swift-buffer-linked-primitives `2d02e17`, swift-buffer-ring-primitives `32e5fd8`/`7f4fc67` |
| **GAP-N** (fold `capacity` + fullness into the SINGLE `Buffer.\`Protocol\``) — *WONTFIX as posed (2026-06-02, principal-adjudicated: "drop capacity from the protocol"); REVIVABLE via GAP-O* | **WONTFIX root:** the STEP-0 verify-on-disk spike found the universal `{capacity, isFull, availableCapacity}` surface VACUOUS for two allocation-only byte buffers the classification omitted — `Buffer.Aligned` (fixed-size, no separate capacity) and `Buffer.Unbounded` (`count` ≡ `capacity`: both `_storage.count.retag(Element.self)`, Buffer.Unbounded.swift:102-109; occupancy is `Binary.Cursor.writerIndex`'s job, doc:28-29) ⇒ `isFull` always-true / `availableCapacity` always-0 (inverted for a resizable buffer). Root cause: `count` means *occupancy* on Arena/Linear/Linked/Ring/Slab but *allocation size* on the 2 byte buffers; `{count,isEmpty}` tolerates the conflation, `{capacity,isFull,…}` exposes it. Disposition: `Buffer.\`Protocol\`` stays the genuinely-universal `{count, isEmpty}`; the orthogonal-to-Storage note (Buffer.Protocol.swift:87-91) is vindicated. **REVIVAL:** if GAP-O reclassifies the 2 byte buffers out of `Buffer.*`, every remaining conformer tracks occupancy-within-allocation and the surface becomes coherent — revisit then. **Prior redesign analysis + vetoed `Buffer.Bounded.Protocol` spike retained below for evidence:** **Redesign:** add `var capacity: Count` (reusing the existing `Count` defaulted associatedtype) + gated (D) `isFull`/`availableCapacity` (`where Count == Index<Element>.Count`, parallel to `isEmpty`) to the one `Buffer.\`Protocol\``; minimal core `Count` bound unchanged (== from the gate, not a core `Equatable`); core identity → `{count, capacity}` occupancy-within-allocation. **Honest value (per the spike classification below):** the gated `isFull = count == capacity` default serves ONLY the disciplines whose fullness *is* `count == capacity` — Ring, Linear (contiguous element-domain); **Arena/Linked/Slab OVERRIDE** with domain-specific fullness (arena free-list+high-water, pool `isExhausted`, bitmap popcount). So the dedup is modest (~Ring/Linear `isFull` + free `availableCapacity`); the primary win is a **uniform occupancy-state surface** (`{count, capacity, isEmpty, isFull, availableCapacity}` queryable on any buffer generically) per the principal's one-protocol directive. Spike wrinkle to settle: Buffer.Linked exposes `capacity` as `Index<Node>.Count` (node-vs-element domain). **— VETOED refinement spike retained for evidence:** The spike compiles: `Buffer.Bounded.\`Protocol\`: Buffer.\`Protocol\` { var capacity: Count }` with `(D) isFull = count == capacity` and `availableCapacity = capacity.subtract.saturating(count)` (Count monus, [IDX-006d]) on the element-domain gate — equality/subtraction on the *refinement*, not the minimal core. Classification narrowed the scope vs. the "~24×, Slab-only-opts-out" expectation: only **Ring** (base/Bounded/Inline) reduces to `count == capacity`; **Arena** (`!hasFree && highWater >= capacity`), **Linked** (`storage.isExhausted`, node-domain capacity), **Slab** (`popcount >= capacity.maximum`) all opt out; **Linear**'s variants don't conform `Buffer.\`Protocol\``. So the dedup is ~3 `isFull` copies (Ring) + net-new `availableCapacity`/`remainingCapacity` — thin [MOD-RENT] rent for a new refinement. Implementation pending the principal's scope decision (Ring-only / proceed / drop) | spike discarded (was uncommitted in swift-buffer-primitives) |
| **GAP-O** (reclassify non-occupancy members out of `Buffer.*`) — *INVESTIGATED 2026-06-02 — Tier-2 RECOMMENDATION `buffer-namespace-membership-occupancy-vs-region.md` (v1.0.0, verified on disk); pending principal go on relocation* | The GAP-N spike surfaced that `Buffer.Aligned` and `Buffer.Unbounded` fail the **buffer-defining property** (occupancy distinct from allocation — they have `count ≡ capacity`); they are raw byte *regions*, not occupancy disciplines (`Aligned` is already built on `Memory.Contiguous` — allocates via `UnsafeMutableRawPointer.allocate(byteCount:alignment:)`, range access from `Memory.Contiguous+RangeAccess`, accepts `some Memory.Contiguous.\`Protocol\``). **Proposal:** relocate them to the **Memory layer** ([DS-006]) as raw-region types (alongside `Memory.Buffer`/`Memory.Contiguous`) — NOT `Memory.Allocator.*` (those are allocators, not allocated regions). Also re-examine `Buffer.Slots` (already a `Buffer.*` non-conformer — metadata-parametric raw slots; Storage-ish). **Payoffs:** (1) `Buffer.\`Protocol\`.count` becomes unambiguously *occupancy* (fixes the conflation); (2) **revives GAP-N** — the occupancy-state surface becomes coherent once the vacuous byte types leave. **Discipline:** class-(c) cross-layer relocation ([ARCH-LAYER-*]/[MOD-DOMAIN]/[DS-020]); the Tier-2 investigation (done) returns these **verdicts** (verified on disk): STAY = the 5 genuine occupancy-over-a-Storage.* disciplines (Linear/Ring→Storage.Heap/Inline, Slab→Storage.Slab, Arena→Storage.Arena, Linked→Storage.Pool); RELOCATE = `Buffer.Aligned` → **Memory** (already a `Memory.Contiguous.\`Protocol\``), `Buffer.Unbounded` → **Memory** (count≡capacity), `Buffer.Slots` → **Storage** (fold into `Storage.Split` — Slots imports/wraps it, "same contract", redundant header; the storage-aware split CORRECTS this doc's earlier Memory-only framing for Slots). **Blast radius verified tiny:** Unbounded 0 dependents, Aligned 1 (Unbounded), Slots 1 (Hash.Table). **CORRECTION ([RES-023]):** this doc's earlier "`Binary.Cursor` uses `Unbounded`" was REFUTED — Unbounded has 0 concrete dependents (that reference was an aspirational doc-comment in Buffer.Unbounded, not a dependency; I'd propagated it unverified). **3 open sub-decisions for the principal** (per the note's Outcome): (a) `Memory.Aligned` distinct sibling vs fold into `Memory.Contiguous`; (b) Slots fold-into vs beside `Storage.Split` (gated by which ops Hash.Table uses — `ensureUnique`/`deinitialize(where:)`); (c) final name for the growable region. Recommended sequence: GAP-O relocations → **revives GAP-N**. Class-(c): principal decides; not dispatched | investigated; relocation pending principal go |
...
- **GAP-C — ~~the `Storage.\`Protocol\``-keyed `span` bridge~~ — SUPERSEDED 2026-06-02 by the North-Star
  De-Pointer arc.** This gap proposed deriving a shared `span` default from `pointer(at:.zero)` + an init-count
  requirement. The North-Star Storage.Protocol de-pointer (`swift-storage-primitives 4aec845`, merged `677ac8f`)
  **deleted `pointer(at:)` from the protocol entirely** — so there is no longer a protocol-level pointer to
  derive a shared `span` from, and the premise is gone. The de-pointer's §4 work instead gave the **contiguous
  conformers (Heap/Inline) a per-discipline `Span`-family surface** (`span` / `mutableSpan` / `outputSpan`), with
  the raw pointer encapsulated inside each conformer body. This is *more* aligned with the model than GAP-C was:
  span is `Memory.Contiguous.\`Protocol\``'s capability, composed per-discipline, NOT a `Storage.\`Protocol\``
  default — exactly the compose-don't-centralize stance. **No action remaining**; the duplication GAP-C cited is
  resolved by the typed/Span surface. (The standing `Experiments/storage-protocol-specialization` still encodes
  the old `pointer(at:)` shape — a doc/experiment follow-up of the De-Pointer arc, not this doc's gap.)
...
- **Hot *read* primitives** (`span`, the Storage `subscript{get set}`, `contains`, `count`) are **requirements** — concrete on the
...
1. **Each layer exposes one minimal capability core** in the ⟨REQUIRES: irreducible primitives⟩ → ⟨PROVIDES:
   ...
   `{capacity, subscript/initialize/move}` (de-pointered); `Buffer.\`Protocol\`` = `count`; the ADT cores add their own identity contract
