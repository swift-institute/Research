...
| 1.23 | new `swift-linter` skill | The 22-item Tier-1 backlog never assigned an owner to the consumer-setup territory (Lint.swift vs Lint/, bundle activation per consumer layer, `excluding(rules:)` Shape γ vocabulary, `// parent:` inheritance, five-package cohort reference). Single existing skill `lint-rule-promotion` covers AUTHOR-side rule promotion; CONSUMER-side adoption was ownership-orphan. **Disposition**: new `swift-institute/Skills/swift-linter/` skill carries `[LINT-SETUP-*]`, `[LINT-BUNDLE-*]`, `[LINT-EXCLUDE-*]`, `[LINT-PARENT-*]`, `[LINT-COHORT-*]`. Default consumer shape is **`Lint.swift`** (single-file) per principal direction 2026-05-18; nested-`Lint/` is the advanced escape hatch (custom rules, non-institute rule packs, per-rule programmatic config). **Companion: swift-linter README §Two consumer shapes / §Adopting / §Internal model rewritten in the same arc** to (a) flip the recommended-default to `Lint.swift`, (b) frame `Lint/` as the advanced shape, (c) codify the asymmetry — `Lint/` is the canonical INTERNAL implementation (both shapes converge at `Lint.Configuration` via `Lint.File.Single.Extractor` + `Lint.Configuration.lift`), `Lint.swift` is the ergonomic front-end built on top of that. | principal direction 2026-05-18 during swift-linter setup-skill scoping | LANDED 2026-05-18 (skill: `swift-institute/Skills/swift-linter/SKILL.md`; README: `swift-foundations/swift-linter/README.md` §Two consumer shapes + §Adopting `Lint.swift` (recommended) + §Adopting `Lint/` (advanced) + §Internal model) |
