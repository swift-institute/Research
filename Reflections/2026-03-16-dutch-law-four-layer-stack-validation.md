...
- Relative paths in Package.swift are brittle across moves. When `rule-burgerlijk-wetboek-2` moved from `~/Developer/rule-law-nl/` to `~/Developer/rule-law/rule-law-nl/`, all relative paths broke. Absolute paths would be more robust but less portable.
