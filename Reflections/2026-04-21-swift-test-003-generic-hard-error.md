...
- [x] Reduce + document the Swift 6.3.1 `SendNonSendable` SIL crash hit during the `unsafe init from borrowing base` test. Minimum reducer captured at `swift-institute/Experiments/sendnonsendable-iife-borrowing-init-crash/` (10 lines across two modules). Identified 6 load-bearing ingredients + 9 ruled-out non-ingredients. Added to `swift-6.3-fix-status.md` "Still Broken on 6.3.1" table. Workaround (bind view at outer scope, no IIFE) is live in the test file with an inline pointer at the experiment. Not filed upstream — deferred to user.
