...
Compiler source reading (local clone at `~/Developer/swiftlang/swift/`): `Collection.map` and `Sequence.map` both `@inlinable @_alwaysEmitIntoClient` with typed throws. Nothing in the attributes themselves would defeat specialization.
