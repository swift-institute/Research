...
Produced: one research-doc "Findings (Modernization)" section appended to `~/Developer/swift-primitives/Research/effect-primitives-and-io-algebra-relation.md`; one package-level reproducer experiment at `swift-effect-primitives/Experiments/noncopyable-optional-capture-crash/` with README and revisit instructions; revisit-trigger doc comment on `Effect.Continuation.One` pointing at the experiment.
