...
**Workspace impact scan post-edit**: `grep -rln "Algebra\.Z\|Algebra_Modular_Primitives" ~/Developer/swift-primitives/*/Sources/ ~/Developer/swift-primitives/*/Package.swift | grep -v swift-algebra` returned **zero** results. O6 confirmed clean: `Array.Bounded.Index` was the only non-algebra source binding ecosystem-wide; the only remaining matches workspace-wide are 10 non-source files (research/docs/Primitives.xctestplan), all categorized (b) per dispatch — no source updates needed.
