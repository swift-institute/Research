...
- Worktree isolation failed: `~/Developer` isn't a git repo, so `isolation: "worktree"` couldn't create git worktrees. Fell back to regular agents, which worked because the packages were independent.
