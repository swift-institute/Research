...
- **`~/Developer/HANDOFF-classification-extension-tier-3.md`** — branching investigation handoff that dispatched this work; supervisor ground-rules block honored; verification stamp at §Outcome.
...
Per the supervisor block's pre-flight requirement, `~/Developer/.claude/skills/` was enumerated before classification began. **52 symlinks total**. Reconciliation against pilot (3) + tier-1 (13) + tier-2 (7) + tier-3 named (24) = **47 in-scope skills**. **9 additional skills accessible via the symlink directory but not in any tier**:
...
| `[RL-CORE-030]` | Legal Package Paths | **mechanical** | Predicate: filesystem → known repositories at the cited paths (`~/Developer/rule-law/`, `~/Developer/rule-law/rule-law-nl/`, `~/Developer/rule-law/rule-law-us-nv/`, `~/Developer/swift-nl-wetgever/`); package-name-pattern → resolution-path mapping table. **Reference (anchor):** definitional anchor for package resolution. | rule-law-core/SKILL.md:150 |
...
Per §Empirical-state pre-flight escalation: 9 additional skills accessible via `~/Developer/.claude/skills/` symlinks but not in the brief's tier-3 list:
...
`~/Developer/HANDOFF-classification-extension-tier-3.md` — branching investigation handoff that dispatched this work; supervisor ground-rules block honored; verification stamp in §Outcome.
