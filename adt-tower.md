...
| `List.Linked` (swift-list-linked-primitives) | carrier `__ListLinked<S, let N: Int>` (already thin-generic!) — W2 adds front doors + deletes the hand `List.Linked.Bounded` type for the alias. **Executed W2 batch-1 (2026-07-05), two recorded refinements**: (1) the hoisted carrier is `__ListLinked<Element: ~Copyable, S: ~Copyable, let N: Int>` — the pre-hoist type nested in the GENERIC namespace `extension List where Element: ~Copyable`, so the hoist makes the inherited `Element` explicit (the §A13/[API-IMPL-009] phantom-generic hoist mechanic; this row's spelling elided it); (2) the `.Bounded` ALIAS is W3-BLOCKED (the `Store.Direct`/`Bounded` twin is conformed by Linear+Ring only, by W1.5 design) — the hand `Bounded` type is KEPT as the recorded W3 residual, alongside two more: the [DS-024] generational-seam analog fixture, and the now-unthrown unbounded `Error` enum (consolidate to the one-carrier-Error house shape at W3's linked op-generalization). Shape-E `throws(.empty)` remove twins deleted per the W2 convention rider. | the `List` namespace root stays (nest for `Linked`) |
...
| `Tree` family | already at-target (carrier `Tree<S>` b09726a); W2 adds the front doors the 6.3.2 SIGSEGV blocked: `Tree<E>.N<n>` etc. via aliases over the tree-n/tree-keyed columns; `TreeDynamic` compound alias RETIRES in favor of the canonical `Tree<E>` front door. **W2 batch-2 execution notes (2026-07-05)**: (1) this row's tree-n premise was FALSE against the tree — `Tree.N` was never ported to the column re-skeleton (no `TreeStorage.N` column exists; ~26-file rebuild mirroring the landed `TreeStorage.Keyed` over `__TreeArena<Element, InlineArray<n, Handle?>>`); RULED deferred to consumer pull (the column has zero possible consumers while RED) — `Tree<E>.N<n>` is BLOCKED-on-re-skeleton, tree-n stays parked-RED n-a; (2) the carrier name-hoists `Tree<S>` → `__Tree<S>` per the classifier + the List.Linked precedent (the row's "at-target" predates the hoisted-name predicate part); (3) `TreeKeyed` retires ALONGSIDE `TreeDynamic` (same compound-alias class; §9.6.5's list was exemplary); (4) `Queue<E>.Shared` / `Stack<E>.Shared` door-pull candidates RECORDED for the W3 review (2 raw-carrier iterator-scratch sites each — below threshold; §9.2.5 interim stands). | tree-n/tree-keyed stay column/sibling packages |
...
| 9 | Hash.Table sentinel tombstones (`empty=0, deleted=Int.min, rehash()` compaction) | MIGRATE to backward-shift + per-instance seed with the W2 engine work. **ALREADY DISCHARGED (verified W2 batch-2, 2026-07-05)**: the migration landed pre-snapshot as part of the ADT-families reshape (in-source attribution: the 2026-06-10 archaeology amendment) — `_shiftChain` backward-shift with rank-plane co-maintenance at `Hash.Table+Removal.swift:58`, per-instance `_seed` (grow-reseeded, clone-preserved) at `Hash.Table.swift:102`, zero `Int.min`/compaction residue, comprehensive differential coverage in place; this ledger row was stale against the tree. One named wrap-around cluster test added at the batch-2 close-out as belt-and-suspenders. |
