...
| Chained `UnsafeMutablePointer<Int>.advanced(by:)` with mixed-direction offsets miscompiles under `-O` / `-Osize` (macOS + Linux) | Not filed (filing held pending user authorization 2026-05-11) | Collapse to a single `.advanced(by:)` call OR bypass user operator overloads OR multiple intermediate `UInt(bitPattern:)` materializations. See `swift-affine-primitives/Research/swift-issue-pointer-arithmetic-workaround.md`. | 8-line standalone `swiftc -O` reproducer at `swift-institute/Issues/swift-issue-pointer-arithmetic-linux-miscompile/UPSTREAM-DRAFT.md`. SIL and LLVM IR byte-identical between affected/unaffected forms — bug lives below SIL in LLVM optimization or backend codegen. Verified macOS arm64 + Linux x86_64 standalone `swiftc -O`; `swift test -c release` masks some configurations on macOS. Investigation arc with false trails (`.Lifetimes`, `unsafe` keyword, operator wrapping — all ruled out) recorded in `swift-institute/Research/swift-issue-pointer-arithmetic-investigation-arc.md`. |
