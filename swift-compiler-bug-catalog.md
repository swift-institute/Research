...
| A13 | `FunctionSignatureOpts` `SILArgument.cpp:40` assertion on a generic fn with a generic typed-throws error | 6.2 → 6.5-dev (`2026-05-27-a`) CRASH; FIXED UPSTREAM on `swiftlang/swift` main 2026-06-08 (`b33ddf2d`/`9adadc68`, `#89617` closed COMPLETED 2026-06-09 by eeckstein) — fix NOT YET in any installed toolchain (newest local dev snapshot `2026-05-27-a` predates it; production manifestations (3)-(5) on 6.3.3-RELEASE, also pre-fix, still crash) | `func f<T>() throws(E<T>)` + same-module caller + `-O` aborts FunctionSignatureOpts creating a `SILArgument` whose type still has a type parameter |
...
| A24 | Debug-mode SILGen `emitValueConstructor` SIGSEGV (signal 11, no assertion text) on a generic `Collection`-typed-throws `init(bytes:)` | 6.3.2 `-Onone` CRASH; 6.5-dev (`2026-05-27-a`/`swift-latest`) INCONCLUSIVE (blocked before the crash site by an unrelated transitive-dependency forward-availability-gate error, see below) | `swift build` aborts with signal 11 (no diagnostic, bare stack dump) while lowering AST→SIL for `RFC_9293.Segment.init<Bytes: Collection>(bytes:)` (`Sources/RFC 9293/RFC_9293.Segment.swift:107`, mangled `@$s15RFC_9293_Shared0A5_9293OABE7SegmentV5bytesAEx_tAE5ErrorOYKcSlRz14Byte_Primitive0F0V7ElementRtzlufC`) — stack top: `ASTLoweringRequest` → `silgen emitValueConstructor SIL function ... for 'init(bytes:)'`. `-Onone` (no `-O` flags in the failing invocation) → distinct from §A13 (release-mode `-O` `FunctionSignatureOpts`); a compile-time frontend crash, not a runtime `swift_getTypeByMangledName` fault → distinct from §A9. No standalone reducer yet (not reduced — cheap-check-only pass) |
| A26 | SwiftPM (not the compiler): graph-load hang — `createResolvedPackages`' conflicting-identity branch calls `findAllTransitiveDependencies` (PR swiftlang/swift-package-manager#8390), an all-paths BFS with no visited set; exponential on dense DAGs | 6.2 → 6.3.2 → SwiftPM `main` (HANGS; not fixed upstream) | `swift build`/graph load spins forever: one thread 97–99 % CPU in `Array.replaceSubrange → _platform_memmove`, zero children, no `.build` writes. Trigger: ONE package identity reachable under two canonical locations (e.g. `.git` vs bare URL spelling across the exact-string mirror table, or old-org vs new-org URLs). Workaround: dual-spelling mirror coverage + uniform `.git` manifest spellings; verify with `swift build` — NEVER `show-dependencies` (its tree dumper is independently exponential). Dossier: `Issues/swift-issue-spm-identity-conflict-path-enumeration-hang/` |
...
**Production manifestation (6)** — `swift-ietf/swift-rfc-9557` (CI-surfaced 2026-07-09/10): `RFC_9557.Suffix.Parse.parse(_:)` (`Sources/RFC 9557/RFC_9557.Suffix.Parse.swift:57` — a generic `Suffix.Parse<Input: Collection.Slice.Protocol>` method whose `throws(Failure)` error `Failure = Parse<Input>.Error` is the accidentally-generic nested enum `expectedOpenBracket`/`unterminatedBracket`/`emptyBracket`, which carries NO `Input`-typed payload; same phantom-generic-error ingredient shape as manifestations (2)/(4)/(5)). Fires under Linux x86_64 `-O -enable-default-cmo`, on BOTH the `Ubuntu (Swift 6.3, release)` Test leg AND the `DocC archive` leg — `Assertion failed: (!type.hasTypeParameter()), SILArgument.cpp:40` in `FunctionSignatureOpts`. macOS debug builds clean (FSO is `-O`-only), which is why local macOS verification did not surface it. First surfaced at package commit `5c882ae`. **REMEDIATED (this pass, 2026-07-10)** — de-phantomed following the `swift-iso-8601` precedent (manifestation (2)): the nested `Parse<Input>.Error` enum hoisted out of the generic namespace to module-scope `__SuffixParseError` (`Sources/RFC 9557/RFC_9557.__SuffixParseError.swift`), the `RFC_9557.Suffix.Parse.Error` spelling preserved via a source-compat typealias (`Sources/RFC 9557/RFC_9557.Suffix.Parse.Error.swift`), and the conformance `Failure` re-pointed at the module-scope type. **Not build-verified locally** — no installed toolchain carries the `#89617` fix and FSO is `-O`-only, so the crash never surfaces in the macOS debug path; consistent with the §A13 upstream-fixed-not-shipping status (the de-phantom is a structural dodge that removes the phantom parameter regardless of toolchain, matching the validated iso-8601 fix).
...
**§A13 Update (2026-07-07) — upstream FIXED, not yet shipping**: `#89617` is **CLOSED** (`stateReason: COMPLETED`, 2026-06-09T18:15:15Z, closed by `eeckstein`). Fix commits `b33ddf2df7e282b37d2233180d93030d145a3b7f` / `9adadc688220b7927db5433590a440d3d9afb3ff` (both dated 2026-06-08T10:28:58Z, message: *"FunctionSignatureOptimization: fix wrong error type when creating a thunk … Fixes a compiler crash https://github.com/swiftlang/swift/issues/89617 rdar://178484080"*) — confirms the root-cause diagnosis above (wrong error type substitution when building the FSO thunk) and lands the fix in `FunctionSignatureTransform::createFunctionSignatureOptimizedFunction()`. Zero comments on the issue; closure was fix-commit-driven, not discussion-driven. **Not yet in any installed toolchain**: the newest local dev snapshot (`2026-05-27-a` / `swift-latest`, self-identifies `Swift 6.5-dev`) predates the 2026-06-08 fix commit by 12 days, so it cannot be used to confirm the fix locally; no post-06-08 snapshot is installed. This is consistent with — not contradicted by — production manifestations (3)/(4)/(5) above, all CI-surfaced 2026-07-06/07 on **Swift 6.3.3-RELEASE**, a *release* toolchain that also predates any downstream release/backport of the 2026-06-08 mainline fix. **Verdict for item 2 of this pass**: upstream signal is unambiguous (closed, fix-commit-linked, root-cause-matching) — no additional release-mode build probe was run (probe is gated on "signal ambiguous" per this pass's brief, and it is not). Re-verify §A13 fix-availability once a post-2026-06-08 snapshot or a 6.3.4+/6.4 release toolchain is installed; the manifestation (3)-(5) production sites remain correctly gated as "NOT remediated (tracking only)" until then.
...
### A24. Debug-mode SILGen `emitValueConstructor` SIGSEGV (signal 11, no assertion text) on a generic `Collection`-typed-throws `init(bytes:)`
...
**Command**: `swift build` (SwiftPM, debug config) in `/Users/coen/Developer/swift-ietf/swift-rfc-9293` (dirty tree — uncommitted lint edits, unrelated to this crash).

**Symptom**: `error: compile command failed due to signal 11` — no assertion text, bare stack dump, while lowering AST→SIL:
```
3.  While evaluating request ASTLoweringRequest(Lowering AST to SIL for file ".../Sources/RFC 9293/RFC_9293.Segment.swift")
4.  While silgen emitValueConstructor SIL function
    "@$s15RFC_9293_Shared0A5_9293OABE7SegmentV5bytesAEx_tAE5ErrorOYKcSlRz14Byte_Primitive0F0V7ElementRtzlufC".
    for 'init(bytes:)' (at .../Sources/RFC 9293/RFC_9293.Segment.swift:107:12)
```

**Location correction**: the crash site is `RFC_9293.Segment.init<Bytes: Collection>(bytes: Bytes) throws(Error)` (`Sources/RFC 9293/RFC_9293.Segment.swift:107`), NOT `RFC_9293.3.1.Header.init(bytes:)` (`Sources/RFC 9293 3 Functional Specification/RFC_9293.3.1.Header.swift:182`) as initially reported — the latter is a plain (non-generic-`Bytes`) `Collection`-typed-throws init in a sibling target that compiles clean; `Segment.init(bytes:)` (same shape, one target up, calling `Header.init(bytes:)` internally) is the actual SIGSEGV site. The mangled symbol's module prefix (`RFC_9293_Shared`) reflects `Segment`'s original declaring module, not the compiling target (`RFC 9293`), which is unremarkable (extension in a downstream target on an upstream-declared type).

**Pattern (observed, not yet reduced to ingredients)**: `public init<Bytes: Collection>(bytes: Bytes) throws(Error) where Bytes.Element == Byte` — a generic function over an associated-type-constrained `Collection`, typed-throws with a **non-generic** nested `Error` enum (does NOT carry the function's generic parameter — ruling out the §A13 shape), whose body `do`/`catch`-remaps an inner call's typed error (`catch let error as RFC_9293.\`3\`.\`1\`.Header.Error`) and calls a sibling generic `init<Bytes: Collection>(bytes:)` internally. No reduction performed (cheap-checks-only pass per [ISSUE-*] discipline); ingredient list not yet verified per [ISSUE-004].

**Distinctness** (per [ISSUE-028]/[ISSUE-031] duplicate-search discipline, cross-checked against §A9 and §A13 per the investigating brief):
- **Distinct from §A13** (`FunctionSignatureOpts` `SILArgument.cpp:40` assertion): §A13 fires only at `-O` in the SIL *optimizer*, on a typed-throws error type that is itself parameterized by the function's own generic parameter (`throws(E<T>)`). This crash fires at `-Onone` (the failing invocation carries no `-O`), during **SILGen** (`ASTLoweringRequest` / `emitValueConstructor`), and the thrown `Error` type (`RFC_9293.Segment.Error`) is a plain non-generic nested enum — the §A13 trigger shape is absent.
- **Distinct from §A9** (`swift_getTypeByMangledName` null-metadata runtime SIGSEGV): §A9 is a **runtime** fault (`TypeLookupError("unknown error")`, execution-time dereference at a fixed small offset) inside a *running* program/test that successfully compiled. This crash is a **compile-time** `swift-frontend` abort during SILGen — no binary is ever produced, and there is no metadata-lookup or generic-witness-table signature in the stack trace.
...
**Evidence**: reproduced live via `TOOLCHAINS=org.swift.632202605101a swift build` in `/Users/coen/Developer/swift-ietf/swift-rfc-9293` (2026-07-07); no experiment or Issues entry created (in-place build only, per this pass's read-only-on-repos constraint).

**Source**: 2026-07-07 issue-investigation cheap-checks pass (two-item toolchain-verification lane).

**Follow-up verification (2026-07-07, same day): PRE-EXISTING, not edit-triggered.** A separate pass isolated whether this crash is introduced by the (still-uncommitted) dirty-tree lint edits — a `shorthand_operator` rewrite (`seq = seq + 1` → `seq += 1`) in `RFC_9293.Segment.nextSequenceNumber`, plus 7 added `public import Binary_Serializable_Primitives` lines, plus cosmetic import-reordering — or whether it pre-dates them.

- A `git worktree add <path> HEAD` build (bare HEAD, no dirty edits) does not reach the crash site at all: it fails first with a genuine, unrelated compile error (`error: 'Serializable' is not a member type of enum 'Binary_Primitive.Binary'`) in `RFC_9293.Port.swift` / `RFC_9293.SequenceNumber.swift`, because those files reference `Binary.Serializable` conformances without importing the module that defines it. The 7 added import lines are a **correctness fix**, not a style whim — HEAD cannot compile past `RFC_9293_Shared` without them.
- Applying *only* those 7 import lines to the HEAD worktree (leaving `seq = seq + 1` untouched, i.e. content equivalent to HEAD minus the unrelated-but-necessary import fix) reproduces **the identical SILGen SIGSEGV signature** (`ASTLoweringRequest` → `silgen emitValueConstructor ... for 'init(bytes:)'`) — this time surfacing at `RFC_9293.3.1.Header.swift:182` (`Header.init<Bytes: Collection>(bytes:)`) rather than `Segment.swift:107`. Since the `shorthand_operator` hunk never touches `Header.swift`, this is conclusive: the crash **pre-exists** the dirty edits.
- The full dirty tree (all edits applied, including the `shorthand_operator` rewrite) was also rebuilt today and reproduces the original `Segment.swift:107` signature exactly as catalogued above.
- **Correction to the "Location correction" paragraph above**: `RFC_9293.3.1.Header.init(bytes:)` is **not** "a plain (non-generic-`Bytes`) Collection-typed-throws init ... that compiles clean." Current source (`Sources/RFC 9293 3 Functional Specification/RFC_9293.3.1.Header.swift:182`) reads `public init<Bytes: Collection>(bytes: Bytes) throws(Error) where Bytes.Element == Byte` — the identical generic shape as `Segment.init(bytes:)` — and crashes with the identical signature when it is the first such initializer the frontend's batch-mode grouping reaches. Which specific call site crashes appears to be **batch/scheduling-dependent** (which files land in the same multi-file `-primary-file` frontend invocation), not a property of one specific declaration — i.e. this bug can fire on *any* declaration matching `init<Bytes: Collection>(bytes: Bytes) throws(Error) where Bytes.Element == Byte` in this codebase, reinforcing the general pattern already recorded above rather than pointing at `Segment.swift` specifically.
- **Disposition**: the dirty tree's lint edits are unaffected by this crash — no hand-revert was performed (would not have helped; the crash reproduces without the `shorthand_operator` hunk present). Park stands.
- **Side finding (out of scope, not acted on)**: `Package.swift` never declares `Binary_Serializable_Primitives` (product of `swift-binary-serializer-primitives`) as a dependency of the `RFC 9293 Shared` or `RFC 9293 3 Functional Specification` targets, even though those targets' sources (both pre-existing and dirty-tree-added) `import` it directly. On a clean `.build`, this produces a nondeterministic `error: no such module 'Binary_Serializable_Primitives'` depending on llbuild job-scheduling order relative to when that module finishes emitting elsewhere in the graph; a second (incremental) `swift build` invocation reliably clears it once the module is cached. This is a genuine missing-dependency-declaration gap, unrelated to §A24 — flagged for the wave-brief owner, not fixed here (Package.swift changes are out of scope for a lint-drain pass and were not authorized).
...
### A26. SwiftPM graph-load hang: identity-conflict diagnostic runs an exponential all-paths BFS (`findAllTransitiveDependencies`)

**Component**: SwiftPM (toolchain, not the compiler). **Versions**: shipped in Swift 6.2 (PR swiftlang/swift-package-manager#8390, 2025-03-25); present and unchanged at `swift-6.3.2-RELEASE`, `release/6.4.x`, and `main` @ `3ff37917a` (2026-07). Not fixed upstream.

**Symptom**: `swift build` / any modules-graph load spins forever after successful version solving — one thread at 97–99 % CPU under `ModulesGraph.load → createResolvedPackages closure #3 → findAllTransitiveDependencies → Array.replaceSubrange → _platform_memmove`; zero child processes, zero `.build` writes, log frozen right after the last "Working copy …" resolution line.

**Trigger**: any package identity *declared* under one canonical location but *resolved* under another (and not a root override) enters the conflicting-identity diagnostic branch (`ModulesGraph+Loading.swift` ~455–470 at 6.3.2). On mirrored machines this happens whenever URL spellings mix: `DependencyMirrors.mirror(for:)` is an exact-string lookup, so `…/pkg.git` (mirrored → local path) vs `…/pkg` (miss → remote) yields two canonical locations for one identity. The branch then runs `findAllTransitiveDependencies` twice per root — an all-paths BFS with **no visited set** and `Array.removeFirst()` pops (O(n) memmove each), enumerating every distinct path from the root through the whole DAG: exponential in depth on dense layered graphs, effectively non-terminating at institute scale (~110–340 packages). One conflicting edge suffices (proven by instrumented 6.3.2 build: print + skip patch).

**Separate co-symptom**: `swift package show-dependencies` is ALSO exponential on dense DAGs via `PlainTextDumper`/`FlatListDumper.recursiveWalk` (prints the DAG once per path; a conflict-free ~110-package graph wrote 41.7 GB without finishing). Never use `show-dependencies` to verify the fix — use `swift build`.

**Workaround (validated 2026-07-10)**: make every spelling of every mirrored identity map to one canonical location — carry BOTH `.git` and bare mirror keys (plus legacy-org spellings) for every entry; the mirror tooling now emits both. Durable fix: uniform canonical-org `.git` spellings in institute manifests (lint candidate). Synthetic offline reproducer (39 local git repos, 2^18-path lattice, two clones of one leaf): `Issues/swift-issue-spm-identity-conflict-path-enumeration-hang/evidence/gen-synthetic.sh` — CONFLICT=1 hangs, CONFLICT=0 builds in seconds.

**Dossier**: `swift-institute/Issues/swift-issue-spm-identity-conflict-path-enumeration-hang/` (terminal record — not filed, standing policy). Sibling record: `swift-issue-spm-planning-build-stall/` (planning-stage variant, 2026-05).

**Source**: 2026-07-10 issue-investigation session (overnight hang-bisect probes a–f + instrumented SwiftPM A/B + synthetic reduction).
