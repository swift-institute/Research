...
- *Decisive cost gate, before any productization* (`[RES-023]`): the spike proved the mechanism **concretely** — a concrete view over a concrete index, with the real `Comparison.Protocol & ~Escapable` Index bound **deliberately dropped** to isolate the lifetime question from the SE-0499 confound. The productization shape is **generic**: a `Collection.Slice.Scoped.Protocol` over `Base: Collection.Protocol`, the slice as a protocol *requirement*, `@_lifetime` threading through **generic dispatch** over the real `~Escapable` Index — and *that* case is **untested**. It, not consumer demand, decides whether productization is a small additive change or a language-edge arc: concrete viability does **not** imply generic-dispatch viability — the same-arc `Iterable.allSatisfy` affordance compiled concretely yet failed through generic protocol dispatch on untied associated types (fixed only by tying `Iterator.Element == Element` on `Collection.Protocol`). The honest first step is therefore a *generic-dispatch spike* (extend the experiment: generic `Base`, slice-as-protocol-requirement, one sample `~Escapable`-index conformer) — cheap, and it converts 'unknown cost' into trivial-or-arc — not a production attempt.
...
- **Empirical baseline + probe**: `swift build` matrix (6.3.2 GREEN / 6.5-dev FAIL→GREEN-under-A), Apple Swift 6.3.2 + 6.5-dev (`org.swift.64202605121a`), arm64-apple-macos26.0, collection-primitives@`ad59898`, 2026-06-02; probe at `[temporary-path]/slice-escapable-probe/`.
