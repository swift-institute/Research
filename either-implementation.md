...
`Either<Left, Right>` was added to `swift-algebra-primitives` as the binary coproduct type -- the categorical dual of `Pair`. The current implementation lives at `~/Developer/swift-primitives/swift-algebra-primitives/Sources/Algebra Primitives/Either.swift`.
