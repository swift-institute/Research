...
- `Binary.Reader<Storage>` is a structural sibling of `Binary.Cursor<Storage>` — same package, same `Span.Protocol & ~Copyable` Storage genericity, same `Index<Storage>` position discipline, same lifetime class. Fills the (owned, read-only, generic-Storage) cell adjacent to Binary.Cursor's (owned, read-write, generic-Storage). Reinforces the *owned reader-writer family*; does not introduce a new axis.
...
| 2 | `Binary.Cursor<Storage>` | `swift-binary-primitives` | `Binary.Cursor.swift:42` | ~Copyable | Escapable | `Storage: Span.Protocol & ~Copyable where .Element == UInt8` | `Index<Storage>` (dual: reader + writer) | Read-write |
| 3 | `Binary.Reader<Storage>` | `swift-binary-primitives` | `Binary.Reader.swift:42` | ~Copyable | Escapable | `Storage: Span.Protocol & ~Copyable where .Element == UInt8` | `Index<Storage>` | Read-only |
...
| `Span.Protocol` | `swift-span-primitives` | The Storage protocol owned cursors are generic over. |
...
**Storage-protocol-backed** (`Storage: Span.Protocol & ~Copyable where .Element == UInt8`): generic over institute Buffer / Memory storage abstractions. Allows the owned cursor cluster to ride on `Buffer.Heap`, `Buffer.Linear`, `Buffer.Aligned`, etc.
...
**Cluster A — Owned ~Copyable reader-writer family** (Binary.Cursor + Binary.Reader). Generic over `Storage: Span.Protocol & ~Copyable`. Index<Storage> position discipline. Live in `swift-binary-primitives`. `Escapable` because they own the storage; can be moved across scopes.
...
| SE-0427 | Noncopyable Generics | Accepted | Enables `~Copyable` on generic parameters; load-bearing for `Index<Element: ~Copyable>` and `Span.Protocol & ~Copyable` Storage. |
...
This is structurally identical to `Binary.Cursor<Storage>` (institute). The institute Binary.Cursor's design *is* the NIO ByteBuffer model applied to the institute's Span.Protocol substrate.
...
- Compose with `Span.Protocol & ~Copyable` Storage abstractions (Buffer.Heap, Buffer.Linear, Buffer.Aligned, …).
...
extension Cursor {
    ...
    public struct OwnedReader<Storage: Span.`Protocol` & ~Copyable>: ~Copyable
    ...
    public struct OwnedReaderWriter<Storage: Span.`Protocol` & ~Copyable>: ~Copyable
    where Storage.Element == UInt8 {
...
In contrast, Shape γ keeps `Binary.Cursor` / `Binary.Reader` in `swift-binary-primitives` (Tier ~10) and the World 2 unified primitive in its own home (lower tier — it doesn't need `Span.Protocol` because World 2 is hard-coded to `Swift.Span<UInt8>`). No single package needs to sit at Tier 13 to absorb all cursor logic.

**Split-package mitigation under Shape ι**: if the tier elevation is the primary concern, the cursor types could split:

- `swift-cursor-span-primitives` (tier ≈ 8) — hosts only World 2's `Cursor.Span<DomainTag>`. No `Span.Protocol` dep.
...
2. **Tier elevation is post-publication irreversible.** A W2-only cursor primitive does not need `Span.Protocol` (it's hard-coded to `Swift.Span<UInt8>`); it sits at Tier 6-8 in the primitives DAG (depends on Tagged, Ordinal, Cardinal, Affine, Index). This is broadly reusable — Buffer-primitives at Tier 10, Binary-primitives at Tier 10, Lexer-primitives at Tier 10 can all consume it. A cursor-primitives package hosting Worlds 1 (which need `Span.Protocol` at Tier 12 as the Storage bound) sits at Tier 13, cutting off every sub-Tier-13 consumer. The Tier 13 commitment is post-publication-irreversible; Shape γ avoids it entirely by leaving W1 in its current Tier 10 home.
...
- Tier: 6-8 in the primitives DAG. Deps: `swift-tagged-primitives`, `swift-ordinal-primitives`, `swift-cardinal-primitives`, `swift-index-primitives` (Test Support spine only). NO `Span.Protocol` dep.
...
- `swift-primitives/swift-binary-primitives/Sources/Binary Cursor Primitives/Binary.Cursor.swift:42` — `public struct Cursor<Storage: Span.Protocol & ~Copyable>: ~Copyable where Storage.Element == UInt8`
- `swift-primitives/swift-binary-primitives/Sources/Binary Cursor Primitives/Binary.Reader.swift:42` — `public struct Reader<Storage: Span.Protocol & ~Copyable>: ~Copyable where Storage.Element == UInt8`
