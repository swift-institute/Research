...
| Phase 3 | swift-foundations/swift-kernel | **No source edits** — terminal gate verification only. Docker Linux swift:6.3.1 `swift build --build-tests` clean on swift-kernel `"Kernel File"` target (the Cycle 3 gate (d) blocker). Confirms redeclaration is resolved at the consumer-module-composition boundary. | HARD gate per Principal Decision 7. | Principal confirms both gates clean (macOS + Docker Linux across iso-9945 + linux-standard + swift-kernel). |
...
**Expected commit graph**: 3 commits across 3 repos (Phase 1 iso-9945 main, Phase 2 linux-standard main, Phase 4 swift-institute/Audits main). Phase 0 + 0.5 + 3 produce no commits. **Effort**: ~3–4 hours implementation + gate verification.

**io_uring consumer under Option A**: unchanged — `UnsafeMutablePointer<Kernel.Signal.Information>` at `Linux.Kernel.IO.Uring.Submission.Queue.Entry+Prepare.swift:1369` now resolves unambiguously to the unified A with `init()` available for zeroed-buffer construction.

**Supervision**: subordinate self-supervises against a `/supervise` ground-rules block (per `[SUPER-002]`) authored after Phase 0.5 grep results confirm accessor disposition. Block scope-locked to Option A + stamped decisions + unused-accessor disposition; MUST/MUST-NOT entries include: (a) no B-declaration left behind (implicit via Phase 2 deletions); (b) no io_uring consumer parameter edit (the zero-migration claim is the Option A zero-migration proof); (c) `.fault` untouched; (d) `ask:` for any deviation. Intervention points per the table's Intervention column — report at each boundary, await Principal confirmation before proceeding to the next phase.
