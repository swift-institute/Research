...
status: RECOMMENDATION (W0–W2 EXECUTED + pushed; W3 decisions principal-ratified + per-conformer taxonomy mapped 2026-06-29; execution in progress under executor authorization; W4 gated)
tier: 2
scope: ecosystem-wide
changelog:
  - "1.4.0 (2026-06-29): OQ1 shape REFINED by full per-conformer mapping (65 conformers / 22 packages): two tiers — FREE-serializer (T 37 stored-String-RawRep + B 1 [Byte]-RawRep = 38; canonical [ASCII.Code] serializer free from the W0 RawRep defaults) and BESPOKE-serializer (P 17 non-RawRep + 'other' 8 computed-String-RawRep-would-be-circular + D 2 dual = 27; must author a real [ASCII.Code] serializer ported from the existing serialize body). The ratified 'retarget' shape applies cleanly ONLY to the free tier. Correctness traps registered: rfc-2045 ContentType double-serializer (unsorted/unquoted serialize body vs sorted/quoted [Byte] init — port the latter; correctness decision), rfc-5322 EmailAddress LinearLifetimeChecker compiler-crash workaround (preserve), rfc-6068 Mailto+Header coupling (migrate together), composite serializers delegating to sub-serializers (LocalPart, RFC_1123.Domain) must land dependencies first. Class D = Option A ratified (keep explicit binary Binary.Serializable + bespoke ASCII serializer). OQ2 deferred-shape wording CORRECTED per principal + family-codable-contextual-parsing.md@4e80c40: the context mechanism is a [FAM-007] context-bearing SUB-SIBLING (refinement of the flat read marker carrying associatedtype Context: Sendable = Void + Error, context on a whole-buffer read, never on Binary.Parseable.parse(from:)) — NOT an associatedtype on the flat top-level marker ([FAM-001] violation). Defer reinforced (zero existential consumers). R1 read-side RATIFIED 2026-06-29: read = flat ASCII.Parseable marker + concrete whole-buffer init(ascii:) (~60 Void conformers; drop the vestigial 'in context: Void' + migrate callers passing 'in: ()') / init(ascii:in:) (5 non-Void); do NOT conform Binary.Parseable (it is a streaming/cursor read fitting self-framing types like UInt32; the variable-length delimiter-framed whole-buffer RFC ASCII types have no natural stream-end → a forced parse(from:) is greedy/semantically-wrong with zero consumers). Streaming-parse where a type genuinely needs it = the dedicated Parser.Protocol/witness layer's job, flagged as a parser-layer note. Plan read-side spec corrected ': Binary.Parseable, ASCII.Parseable' -> ': ASCII.Parseable' + concrete reader. keep-Binary.Serializable/skip-Binary.Parseable asymmetry is principled (produce-bytes always well-defined; streaming-consume needs framing these types lack)."
...
L2 swift-ietf/* · swift-whatwg      each conformer: Serializable + ASCII.Serializable  ∥  ASCII.Parseable (flat marker) + concrete init(ascii:) [R1]
...
### OQ1 refinement (v1.4.0) — per-conformer taxonomy (65 conformers / 22 packages, mapped)

The ratified "retarget the hand-written `serialize(ascii:into:)`" shape holds **only** where the canonical `[ASCII.Code]` serializer is supplied free by a W0 RawRep default. A read of all 65 conformers splits them into two tiers:

**Free-serializer tier — 38** (canonical serializer free; ~mechanical):
- **T (37)** — stored `RawValue == String`, ASCII serialize = `rawValue.utf8`. Planned shape (pathfinder-validated before fan-out): `: Serializable, ASCII.Serializable, Binary.Serializable, Swift.RawRepresentable` + re-provide `init?(rawValue:)` (currently inherited from the deprecated `Binary.ASCII.RawRepresentable`) + an explicit disambiguating `serialize(_:into:)`; delete the now-redundant `serialize(ascii:into:)`; read side per **R1** below.
- **B (1)** — stored `RawValue == [Byte]` (rfc-2046 Content). Serializer free from the `[Byte]`-RawRep default; this is the OQ1-probe-verified case.

**Bespoke-serializer tier — 27** (must author a real `[ASCII.Code]` serializer, ported from the existing serialize body):
- **P (17)** — not `RawRepresentable`; single ASCII serialization; bespoke serialize. (rfc-2046 ×3, rfc-2183 ContentDisposition, rfc-2387 Related, rfc-4007 ScopedAddress, rfc-5322 ×5, whatwg-url ×6.)
- **"other" (8)** — `Binary.ASCII.RawRepresentable` with `RawValue == String`, but the rawValue is **computed FROM serialization** (no stored String) → the String-RawRep free serializer would be **circular**, so these behave like P: rfc-2045 ContentType, rfc-5321 EmailAddress, rfc-5322 EmailAddress, rfc-6068 Header + Mailto, rfc-7519 JWT, rfc-7617 Basic + Challenge.
- **D (2, Option A ratified)** — dual binary+ASCII: rfc-791 IPv4 (`UInt32`), rfc-4291 IPv6 (8×`UInt16`). Keep the explicit binary `Binary.Serializable` (out-ranks the bridge); author a bespoke `[ASCII.Code]` text serializer.

**Per-conformer correctness traps (ground-rule #6a — resolve/surface during execution):**
- **rfc-2045 ContentType** — two non-identical serializers: the `serialize(ascii:into:)` body (unsorted params, values unquoted) vs the `[Byte](ContentType)` init (params sorted by key, values quoted on tspecials). The richer/correct form is the `[Byte]` init; the bespoke serializer must port THAT. **Correctness decision (preserve current bytes vs adopt the canonical sorted/quoted form).**
- **rfc-5322 EmailAddress** — reader carries a LinearLifetimeChecker compiler-crash workaround (generic init delegates to a concrete `init(ascii: [Byte])`); preserve it.
- **rfc-6068 Mailto + Mailto.Header** — coupled (Mailto's serialize invokes Header's); migrate together.
- **Composite serializers** (rfc-5321/5322 EmailAddress, rfc-6068) delegate to sub-component serializers (LocalPart, RFC_1123.Domain); the sub-serializers must also be in `[ASCII.Code]` form → land dependencies first.

**Read-side shape (R1, principal-ratified 2026-06-29) — applies to ALL conformers:** conform the **flat `ASCII.Parseable` marker only** + keep the concrete whole-buffer reader; **do NOT conform `Binary.Parseable`**. ~60 Void conformers → `init(ascii:)` (drop the vestigial `in context: Void`; migrate any caller passing `in: ()`); the 5 non-Void → `init(ascii:in:)`. Rationale: `Binary.Parseable.parse(from:)` is a streaming/cursor read that fits self-framing types (`UInt32` = 4 bytes); the RFC ASCII types are variable-length, delimiter-framed, whole-buffer values with no natural stream-end, so a forced `parse(from:)` is greedy/semantically-wrong with zero consumers. R1 matches the deprecated protocol (whole-buffer read only — no regression), the OQ2 conclusion (concrete reader is first-class; markers are for dispatch that doesn't exist yet), and probe F (context on whole-buffer, never on `parse(from:)`). The keep-`Binary.Serializable`/skip-`Binary.Parseable` asymmetry is principled: produce-bytes is always well-defined; streaming-consume needs framing these types lack. **Streaming-parse, where any single type genuinely needs it, is the dedicated-parser (`Parser.Protocol`/witness) layer's job — NOT `Binary.Parseable` on the value; flag such a type as a parser-layer note.**

**Non-Void readers (5, OQ2 — keep concrete `init(ascii:in:)`):** rfc-2046 Multipart, rfc-2387 Related, whatwg-url URL/Host/Path.
...
- **DEFERRED context shape (principal correction, v1.4.0)**: a **[FAM-007] context-bearing SUB-SIBLING** — a refinement of the flat read marker carrying `associatedtype Context: Sendable = Void` (+ `Error`), Void-default + context-free convenience — which is the deprecated `Binary.ASCII.Serializable`'s ACTUAL structure. **NOT** an `associatedtype` on the flat top-level `ASCII.Parseable`/`Binary.Parseable` marker (that violates [FAM-001]). Context lives on a whole-buffer read, never on `Binary.Parseable.parse(from:)`. Full reasoning + pre-resolved design + build-trigger: `Research/family-codable-contextual-parsing.md`. (The earlier "associatedtype Context = Void on the existing flat marker" wording was wrong — shape-1 is a hard [FAM-001] violation; the cited "proven shape" already *refined* `Binary.Serializable`, i.e. it was structurally a FAM-007 sub-sibling. [FAM-007] worked example: `UInt8.Base62.Serializable`.)
