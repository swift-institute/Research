...
- **T (37)** — stored `RawValue == String`, ASCII serialize = `rawValue.utf8`. Planned shape (pathfinder-validated before fan-out): `: Serializable, ASCII.Serializable, Binary.Serializable, Swift.RawRepresentable` + re-provide `init?(rawValue:)` (currently inherited from the deprecated `Binary.ASCII.RawRepresentable`) + an explicit disambiguating `serialize(_:into:)`; delete the now-redundant `serialize(ascii:into:)`; read side `: Binary.Parseable, ASCII.Parseable`.
