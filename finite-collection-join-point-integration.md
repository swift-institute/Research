...
## Addendum: CaseIterable Constraint and Decision Not to Pursue (2026-02-10)

### Tier Correction

The tier numbers in this document (finite at Tier 11, collection at Tier 10) are from
an earlier tier computation. Freshly computed tiers (2026-02-10) show:

- `finite-primitives`: **Tier 7** (max dep: `index-primitives` at Tier 6)
- `collection-primitives`: **Tier 8**
- After extraction: finite would drop to **Tier 6**, cascading dimension 8→7, time/complex/algebra-linear/region 9→8

The dependency causing tier inflation is `index-primitives` (not `collection-primitives`
as stated in the original document). The `index-primitives` dependency exists for two files:

1. `Finite.Enumeration.swift` — Collection conformance uses `Index<Element>` as `Swift.Collection.Index`
2. `Index.Bounded.swift` — bridging between `Index<Element>` and `Ordinal.Finite<N>`

### Overlooked Constraint: CaseIterable Requires Collection

This research overlooked a critical constraint in the `CaseIterable` protocol:
...
`Finite.Enumerable` inherits `CaseIterable` (line 36 of `Finite.Enumerable.swift`).
The default `allCases` returns `Finite.Enumeration<Self>`. For CaseIterable's
`AllCases: Collection` requirement to be satisfied, `Finite.Enumeration` **must** be a
`Collection` in the same module as the protocol declaration.

Moving Collection conformance to a separate package breaks this chain:

- `Finite.Enumerable: CaseIterable` can no longer be declared in `finite-primitives`
- All 9 comparison/ordering types lose CaseIterable (inherited via protocol hierarchy)
- The `Tagged` blanket CaseIterable conformance breaks
- All `@Witness` macro-generated types lose CaseIterable (generated as `: Finite.Enumerable`)

### Approaches Investigated

**Approach A: Int-based Collection (keep CaseIterable)**

Change `Finite.Enumeration`'s `Collection.Index` from `Index<Element>` to `Int`.
Removes `index-primitives` dependency. CaseIterable stays. New package provides typed
`Index<Element>` as an optional enhancement.

- Experiment: CONFIRMED (all variants pass)
- Rejected: downgrading from typed `Index<Element>` to `Int` violates [IMPL-002]

**Approach B: Move Enumeration entirely (principled)**

Remove `CaseIterable` from `Finite.Enumerable`. Move `Finite.Enumeration` (with
typed `Index<Element>` Collection) to new package. Provide retroactive CaseIterable
for each concrete type. Update witness macro.

- Experiment: CONFIRMED (all variants pass, including cross-module witness matching
  and retroactive CaseIterable)
- One finding: `Sequence.underestimatedCount` returns 0 through generic `CaseIterable`
  dispatch due to cross-module witness table limitation (`.count` works correctly)

### Decision: NOT PURSUED

After full analysis, the extraction was deemed not worth the cost:

1. **`allCases` stops being automatic** — conforming to `Finite.Enumerable` no longer
   gives `.allCases` without a second import
2. **Witness macro must change** — must generate `: Finite.Enumerable, CaseIterable`
   (two conformances) and the consumer module must import the new package
3. **Retroactive CaseIterable maintenance burden** — every new `Finite.Enumerable` type
   needs a corresponding retroactive CaseIterable in a different package
4. **`Finite.Enumeration` becomes half-useful** — Sequence-only in finite-primitives,
   Collection only with the integration import
5. **`underestimatedCount` cross-module witness table bug** — `Array(T.allCases)` through
   generic CaseIterable won't pre-allocate (Swift compiler limitation)
6. **Narrow real benefit** — only `algebra-modular` and `algebra-law` don't need
   collection features; buffer/hash-table/set/dimension all need the integration package

The tier compression (5 packages drop one tier) does not justify splitting
`Finite.Enumerable` into a two-import experience.

### Experiment

Validation experiment at `Experiments/finite-collection-extraction/`. Tests both
approaches across 5 variants: retroactive CaseIterable, macro-generated CaseIterable,
typed `Index<Element>` access, cross-module witness matching, and Sequence-only usage.
