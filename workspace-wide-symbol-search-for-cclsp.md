...
Claude Code sessions start from `~/Developer`, a local-only directory that is not a git repository and does not exist on GitHub. The Swift Institute ecosystem lives in sub-directories (swift-primitives/, swift-standards/, swift-foundations/), each containing monorepos of independent Swift packages.
...
It always sends **one** workspace folder: `rootDir` if configured, otherwise `process.cwd()`. Since `rootDir` isn't set and CWD is `~/Developer` (no Package.swift), SourceKit-LSP gets a workspace root with no discoverable build system.
...
**Constraint**: Implicit discovery only searches within the explicit workspace root. If the root is `~/Developer`, any sub-directory's Package.swift is discoverable. If the root were `~/Developer/swift-primitives/swift-buffer-primitives`, siblings would not be.
...
**What changes**: SourceKit-LSP gets `~/Developer` as the explicit workspace root. No Package.swift there, so no SwiftPM workspace is created at the root level. But implicit workspace discovery now covers the entire Developer tree — any file opened in any sub-package triggers an implicit workspace for that package.
...
Add both:
1. `rootDir: "~/Developer"` in cclsp.json
2. A `Package.swift` at `~/Developer` declaring dependencies on key packages

```swift
// ~/Developer/Package.swift
  ...
  "servers": [
    {
      "extensions": ["swift"],
      "command": ["sourcekit-lsp"],
      "rootDir": "~/Developer/swift-primitives"
    },
    {
      "extensions": ["swift"],
      "command": ["sourcekit-lsp"],
      "rootDir": "~/Developer/swift-standards"
    },
    {
      "extensions": ["swift"],
      "command": ["sourcekit-lsp"],
      "rootDir": "~/Developer/swift-foundations"
    }
  ]
}
```
...
Combine with Option A (`rootDir: "~/Developer"`) for workspace root.
...
Place a BSP connection file at `~/Developer/.bsp/swift-institute.json`:
