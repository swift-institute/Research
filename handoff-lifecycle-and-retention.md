...
26 such files at `~/Developer/` (the brief stamped 25 at 11:30;
...
**Constraints**:
- `~/Developer/` is NOT a git repository — deletions are
...
- `~/Developer/` is not a git repository (verified per CLAUDE.md
...
Move HANDOFF-*.md files from `~/Developer/` (non-git-tracked) to
...
| Side effects | Discoverability hit. Working-tree contamination resolves (gitignore removes the `~/Developer/HANDOFF*.md` clutter), but the migration cost is paid once for a benefit that's marginal vs. Option B. |
...
- **Durable record of the prior triage cycle**:
  - `~/Developer/HANDOFF-handoff-files-triage-and-cleanup-table.md`
